...
void TrainSvm::test()
{
    ...
    vector<string> test_hasplates;
    vector<string> test_noplates;
    getFiles(test_pos_plates_folder,test_hasplates);
    getFiles(test_net_plates_folder,test_noplates);
    Ptr<SVM>model = SVM::load(svmxml_);
    vector<int>traingingLabels;

    for(auto imgsuffix:test_hasplates)
    {
        Mat img = imread(test_pos_plates_folder+"/"+imgsuffix,0);
        resize(img,img,Size(platewidth,plateheight),(0, 0), (0, 0), INTER_LINEAR);
        img = img.reshape(1,1);
...
int main()
{
    const string folder  = "/home/mxq/Code/OpenCVLinux/CVPlateSVM/resource/data";
    const string xml = "plates.xml";
